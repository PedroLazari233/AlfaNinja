...
#pragma warning disable
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (isFeasibleTargetForPolyFew)
            {
                ...
                if(isPlainSkin)
                {
                    if (GUILayout.Button("<size=14><b>POLY FEW</b></size> <size=7><b>v7.61</b></size>", style)) { toolMainFoldout = !toolMainFoldout; }
                }
                else
                {
                    if (GUILayout.Button("<size=14><color=#A52A2AFF><b>POLY FEW</b></color></size> <size=7><b><color=#A52A2AFF>v7.61</color></b></size>", style)) { toolMainFoldout = !toolMainFoldout; }
                }
                ...
                if (GUILayout.Button(content, GUILayout.Width(24), GUILayout.Height(24)))  
                {
                    if (lastRect != null)
                    {
                        ...
                        content = new GUIContent();
                        content.text = "Enable Auto UI Attaching";
                        content.tooltip = "Enable the automatic popping up of PolyFew UI for all feasible GameObjects. This doesn't attach PolyFew to every object in your scene.";

                        definitions.Add(new PopupToggleTemplate.ToggleDefinition(content, 190, -4, (bool value) =>
                        {
                            EditorPrefs.SetBool("polyfewAutoAttach", value);
                            if (value) { InspectorAttacher.AttachInspector(); }
                        },
                        () =>
                        {
                            return EditorPrefs.GetBool("polyfewAutoAttach", true);
                        }));


                        content = new GUIContent();
                        content.text = "Disable Auto UI Attaching";
                        content.tooltip = "Disable the automatic popping up of PolyFew UI. You would have to manually attach PolyFew to GameObjects. Please note that once you attach PolyFewHost it will still get removed from the GameObject when it looses focus. This is not the recommended behavior as PolyFew is not designed to work this way.";
                ...
                if (toolMainFoldout)
                {
                    ...
                    if (prevValue != ConsiderChildren == true)
                    {
                                        ...
                                        if (mRendererPair.attachedToMeshFilter)
                                        {
                                            ...
                                            if (filter != null)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (OverwriteAssetAtPath(mRendererPair.mesh, moddedMesh, true)) { }
                                                    else
                                                    {
                                                        mRendererPair.mesh.Clear();
                                                        EditorUtility.CopySerialized(moddedMesh, mRendererPair.mesh);
                                        ...
                                        if (originalRendererPair.attachedToMeshFilter)
                                        {
                                            ...
                                            if (filter != null)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    //Overwrites the mesh assets and keeps references intact
                                                    if (OverwriteAssetAtPath(originalRendererPair.mesh, moddedMesh, true)) {  }
                                                    else
                                                    {
...
#pragma warning disable
        ...
        public void DrawBatchFewUI(bool boxDrawnAlready = false)
        {
            ...
            if (FoldoutBatchFew)
            {
                ...
                //    }
                //}
                canCombineMeshes = Selection.gameObjects != null && Selection.gameObjects.Length > 0;
                ...
                EditorGUI.BeginDisabledGroup(Selection.gameObjects == null || Selection.gameObjects.Length != 1);
                ...
                if (didPressButton)
                {
                    ...
                    else if (Selection.gameObjects.Length > 1)
                    {
                        ...
                        if (anySuccess)
                        {
                            ...
                            if (CreateAsChildren)
                            {
                                message = $"All renderers in the target GameObjects have been disabled. A copy of each original object with the combined mesh(es) has been created as a child to the corresponding target object. Please note that the root bones for the combined skinned mesh renderers should not be deleted in the new objects. Any changes to prefabs must be manually applied.";
                            }

                            else
                            {
                                message = $"The original GameObjects have been disabled. Copies of the original objects with combined mesh(es) have been created. Please note that the root bones for the combined skinned mesh renderers should not be deleted in the new objects. Any changes to prefabs must be manually applied.";                            
                            }
...
#region COLOR SPACE
        ...
        private void DestroyPolyFewHost()
        {
            ...
            if (thisGameObject != null)
            {
                ...
                if (host != null)
                {
                    SystemServices.RunDelayedCommand(0.1f, () =>
                    {
                        try { DestroyImmediate(host, true); }
                        catch (Exception ex)
                        {
        ...
        private void ApplyExtraOptionsForLOD(GameObject lodObject)
        {
            ...
            {
                var backup = lodObject.GetComponent<UnityMeshSimplifier.LODBackup>();
                if (backup) { DestroyImmediate(backup); }
            }
